...
const myexp = ((request, response) => {
    ...
    const alcoholComposing = async () => { //กรณีอยากรู้ปริมาณการขับออก
        ...
        return agent.add(createQuickReply('แล้วคุณอยากรู้ไหมคะ ว่าต้องใช้เวลารอนานแค่ไหน กว่าที่ระดับแอลกอฮอล์ในเลือดของคุณจะลดต่ำลงกว่า 50 มิลลิกรัมเปอร์เซ็นต์ ซึ่งหากไม่มีการดื่มเพิ่มระหว่างนั้นจะถือว่าเป็นระดับที่ไม่ผิดกฎหมายถ้าคุณขับขี่ และมีอายุเกิน 20 ปี', ["อยากรู้", "ไม่อยากรู้", "แก้ไขข้อมูลแอลกอฮอล์"]));
    }
    ...
    const alcoholLessThan = async() => { //กรณีอยากรู้ เวลาลดลงของแอลกอฮอล์
        ...
        agent.add(`ถ้าหากข้อมูลการดื่มและข้อมูลผู้ดื่มเป็นไปตามที่แจ้งไว้ และเวลาไป ${time} ชั่วโมง ระดับแอลกอฮฮล์จะเหลืออยู่ ${result} มิลลิกรัมเปอร์เซ็น`);
    ...
    const knowladgeSection = async () => {
      ...
      const Test = async () => {
            ...
            return agent.add(new Payload('LINE', imageCarousels.alcohol().containerSize["alcohol"], { sendAsMessage: true }));
